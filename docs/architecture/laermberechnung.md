...
| `distribution.ts`   | Schritt 2 (7.5, `slm 32`): `distributeShots(menge, quellen[{sourceId, weight}])` verteilt die Menge einer Kombination im Verhältnis der Gewichte auf die Quellen (Schusslinien); **alle Gewichte 0 → Gleichverteilung mit `warning: 'zero-weights'`** (Prüfbericht), eine Quelle → alles, Dezimalmengen auf 3 Dezimalen summentreu. Angebunden, sobald Quellen mit Gewichten je Zustand importiert werden (heute: eine Quelle je Kombination) |
