...
| **Datenverwaltung › Schiessplatz › Allgemein 5.15 / 5.16 (`slm 14`–`16`)** | Mock `_mocks/data-management/area-detail.index.html`, gegen B1 Abbildung 26/27 geprüft: Kontextleiste mit Schiessplatz-Wechsler (Suche Bezeichnung / Koordinationsabschnitts-Nr. / Sachplan-Nr.), Reiter Übersicht / Stammdaten. Übersicht: Detailansicht (Koordinationsabschnitts-Nr., Sachplan-Nr., Aktiv, Berechnungsart Anhang 7, Klassierung, **Baujahr Anlageteile aus dem aktuellen Zustand**, Stände) und Stellungsraum-Tabelle mit Freitextsuche, Sortierung, Räume ohne Koordinationsabschnitts-Nr. (B1-Hinweis, im Demo zwei). Stammdaten: neue Spalten `schiessplatz.classification`, `recalculationState`, `remediationProjectState`, `spmState`, `noiseRemediationState`, `projectState`, `planningApproval` (Codes `area-master-data.enums.ts`), Pflichtfelder, Guard für ungespeicherte Änderungen, Kontingent-CRUD (`POST/PATCH admin/data/area/:id/quota`, Löschen als POST wegen `write`-Recht), ein Kontingent je Kombination (409), Grundlage = Plangenehmigung, Hinweis «zulässige Kombination ohne Kontingent» (Soll 0), Ampel-Refresh, Logbuch-Einträge mit Vorher/Nachher; Lesemodus für Rollen mit `read` (`GET admin/access`) | API `admin-data-area.controller.spec` (7), `admin-access.controller.spec` (4); Jest `dm-area-general-overview` (6), `dm-area-master-data` (10); e2e `dm-area-general.spec` (4) |
| **Datenverwaltung › Waffen 5.22–5.25 (`slm 22`–`25`)** | Mock `_mocks/data-management/weapon.html`, gegen B1 Abbildung 36–39 geprüft; Muster aus ELO `admin-weapon` (Export mit ExcelJS, Logbuch je Mutation, Löschschutz) übernommen: eine Maske, vier Listen (Waffe/Kaliber, Kaliber, Waffe, Waffenkategorie) mit Zählern, Suche, Filter Kategorie / Waffe / Kaliber, Sortierung, Detailformular DE/FR/IT + Felder je Liste (ALN-/SAP-Nr., Einheit Stück/kg, Waffenkategorie, Anhang-7-Kategorie a–f nach LSV Anhang 7 Ziff. 1, Zuordnung sonARMS mit Vorschlägen aus den Schusslinien), Verwendung je Schiessplatz, Erfassung / letzte Änderung; API `admin/data/weapons/*` mit Eindeutigkeit (Bezeichnung DE, Schlüssel, Waffe × Kaliber), abgeleiteter Bezeichnung «Waffe · Kaliber», **Löschschutz** (409 `<kind>-in-use` mit Anzahl über Waffen / Kombinationen / Zuordnungen / Nutzungen / Kontingente / Schusslinien → Maske bietet «Inaktiv setzen»), XLSX-Export (vier Blätter, DE/FR/IT/EN) | API `admin-data-weapons.controller.spec` (7); Jest `dm-weapons` (10); e2e `dm-weapons.spec` (5) |
| **Datenverwaltung › Schiessplatz › Berechnungen 5.18–5.21 (`slm 18`–`21`, `slm 45`)** | Kein Mock vorhanden — Masken direkt aus B1 Abbildung 29–35 abgeleitet. Neues API-Modul `data-calculations` (`admin/data/area/:areaId/calculations/*`, App 42): Übersicht der Immissionsberechnungen mit Zuständen, Lieferung anlegen / bearbeiten / löschen (Löschschutz aktuell / MGDM / Berechnungsläufe, Soft-Delete), Zustand anlegen mit Zustand ID `<KA-Nr>_<n>`, Zeiger über `CalculationService`, **Validierung ohne Schreiben** (`POST …/import/validate` mit Befunden / Warnungen / Zählern; der Import selbst bleibt Staging + Transaktion), Upload WLR je Zeitgruppe (ersetzt die Zeitgruppe, unbekannte Empfänger / Quellen werden gemeldet) und Betriebsdaten Anhang 9 / 7 je QuellenID (Upsert, A7-Kategorie aus der Waffe), Export der Zustände als wieder importierbares JSON-Bündel und Schusszahlen als CSV, Details je Stellungsraum. Dateiparser (`.wlr`, Tab/Semikolon, Spalten-Aliasse) und Export-Bündel sind reine Funktionen in `calculation-files.service.ts`; Fachregeln der Masken in `core/data-calculations/calculations.logic.ts`. Spalten `immissionsberechnung.description` / `fileName`. Frontend: Reiterleiste im Schiessplatz-Kontext, Übersicht mit Zeiger-Bestätigung, Import mit Strukturprüfung → Validieren → Importieren, Export mit Auswahl und Dialog «Neuer Berechnungszustand», Details mit vier Reitern; Lesemodus über `GET admin/access` | API `calculation-files.service.spec` (16), `admin-data-calculations.controller.spec` (12); Jest `calculations.logic.spec` (13), `dm-calc-overview` (11), `dm-calc-import` (9), `dm-calc-export` (7), `dm-calc-details` (6); e2e `dm-calculations.spec` (6) |
| Deep Link nach dem Login (`slm 5`, `slm 6`) | `returnUrl` des Guards wird auf der Login-Seite je Tab geparkt (`views/auth/return-url.ts`, llumi-Muster, `@app-galaxy/auth-ui` 0.0.225 wie llumi) und im Mandanten-Wähler eingelöst — überlebt 2FA / E-Mail-Verifikation / Passwort-Reset; nur interne `/admin`-Pfade | Jest `return-url.spec` (5); e2e `c08-sitemap.spec` (2) |
| Kontingent löschen und neu erfassen (5.16) | Der Unique-Index `kontingent(tenantId, areaId, combinationId)` zählte auch weich gelöschte Zeilen — «löschen, dann neu erfassen» endete in einem 500; `assertCombinationFree` räumt die gelöschte Zeile vor dem Anlegen / Umhängen weg | `admin-data-area.controller.spec` |
| App-Rechte der Sitzung (`GET admin/access`) | Neues Modul `access`: bestes Recht je App über die aktiven Rollen (`app_user_right` × `app_role_right`, Admin-Rolle = root auf allen Apps); `AccessFacade.canWrite(appId)` schaltet Masken in den Lesemodus, `can(appId)` filtert das Menü (vorher `GET admin/apps/app/user/:userId` ohne Rechtsstufe) | `admin-access.controller.spec` |
| Demo-Datensatz v6 | Stammdaten 5.16 je Schiessplatz (Geissalp wie Abbildung 26/27, Hinterrhein inaktiv), zwei Stellungsräume ohne Koordinationsabschnitts-Nr., Kaliber mit SAP-Nr. und FR/IT | `tenant-dataset.spec` |
| e2e-Konfiguration | `testMatch` des Projekts `chromium` erfasste `src/admin/*.spec.ts` nicht (nur `src/*.spec.ts`); die Admin-Specs liefen dadurch nie mit — korrigiert | `apps/app-e2e/playwright.config.ts` |
...
| Angular Jest (`npx nx test app`, 11 Suiten) | **73 bestanden** |
...
| Export | Waffen-Stammdaten XLSX, Zustände JSON, Schusszahlen CSV; Berichte deaktiviert | Excel/PDF-Berichte (`slm 3`, `slm 39`–`41`), FGDB (GDAL) |
...
2. **Datenverwaltung** (5.17 Zuordnung Waffen; 5.14–5.16 und 5.22–5.25 sind umgesetzt): Maske für die zulässigen
   Kombinationen je Stellungsraum; Seed aus B1.6/B1.7 (echte 126 Schiessplätze).
