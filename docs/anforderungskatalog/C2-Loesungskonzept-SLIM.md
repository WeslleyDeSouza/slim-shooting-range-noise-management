...
3. Verteilung auf Quellen: jede Kombination Stellungsraum × Waffe kennt ihre Quellen des Zustands mit Gewicht (Kapitel 2.3); die Schusszahlen aus Schritt 2 werden im Verhältnis dieser Gewichte auf die Quellen (Schusslinien) verteilt – eine Kombination mit genau einer Quelle erhält alles. Kombinationen ohne Gewicht in den Betriebsdaten werden gleichverteilt und im Berechnungsstand als Annahme gekennzeichnet; Kombinationen ohne Quelle werden als nicht zuordenbar ausgewiesen (Bestätigung durch KOMZ Lärm im Refinement; Verhalten Gleichverteilung/Verweigern konfigurierbar). *Prototyp:* Verteilungsschritt im Kern umgesetzt und getestet, inkl. Dezimalmengen.
...
| slm 43 | Berechnungsstruktur je Immissionsberechnung/Zustand (ZustandsID) | 2.3 | P (Quellengewichte Z) |
...
| slm 51 | Deutsche Bezeichnungen der DB-Objekte | 2.3 | P (Tabellen; Spalten Z) |
