...
- [ ] **Vorgabe:** Immissionsberechnungen und deren versionierte Zustände nachvollziehbar modellieren (slm 18/43). — **Befund:** Text 2.3 stimmt («die Zustände tragen Quellen, Empfangspunkte und WLR-Werte»), das ERD in 2.3 widerspricht ihm: `AREA ||--o{ AREA_RECEIVER` hängt die Empfangspunkte am Schiessplatz, ein Anlageteil je Zustand fehlt, Untersuchungsperimeter und Gebäude fehlen. Matrix slm 43 «P (Quellengewichte Z)» ist zu hoch: im Prototyp hängen nur die WLR-Werte am Zustand. B1 Kap. 10 ist verbindlich (Muss): Text, Diagramm und Matrix müssen dasselbe Zielbild zeigen (`validierung-fachlich.md` Abschnitt 7); Matrix auf «Z (WLR-Werte je Zustand P)».
- [x] **Vorgabe:** Jeweils eindeutigen aktuell gültigen Zustand und Stand MGDM verwalten (slm 18). — 2.3 «Datenbank-Constraint» (im Prototyp nur Flags, kein Constraint).
...
- [x] **Vorgabe:** Unbekannte Stellungsräume mit Warnung und Importabbruch behandeln (slm 45). — 3.2/3.3 («Import mit Fehlern übernimmt nichts»).
