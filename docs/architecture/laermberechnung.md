...
| `distribution.ts`   | Schritt 2 (7.5, `slm 32`): `distributeShots(menge, quellen[{sourceId, weight}])` verteilt die Menge einer Kombination im Verhältnis der Gewichte auf die Quellen (Schusslinien); **Σ Gewichte = 0 → Verweigern (Default, `refused: true`, `warning: 'zero-weights'`)**; die Ersatzregel Gleichverteilung (`onZeroWeights: 'equal'`) nur mit dokumentierter Freigabe (`release: {reference, date}`, sonst Fehler) und mit Kennzeichen `substituteRule` (Fachregel O8, KOMZ Lärm). Teilweise Nullen behalten das definierte Verhältnis; **keine Quelle → `warning: 'no-source'`** (Menge «nicht zuordenbar», nie stillschweigend weg). Dezimalmengen auf 3 Dezimalen summentreu. Der Aufrufer (Assessment) setzt betroffene Empfangspunkte auf `incomplete` = «nicht beurteilbar» (keine Ampelfarbe, Teilpegel sichtbar, `missingSources` im DTO) — eine Untergrenzen-Logik («gesichert rot») ist bewusst nicht umgesetzt: für Anhang 7 ist der Rest-Lr wegen −7·log(Mᵢ) und der Halbtage keine Untergrenze. Angebunden, sobald Quellen mit Gewichten je Zustand importiert werden (heute: eine Quelle je Kombination) |
