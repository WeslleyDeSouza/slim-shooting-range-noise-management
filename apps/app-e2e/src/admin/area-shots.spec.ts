...
const SHOTS = {
  ...
  /** First position of the drawer (`#shots-category-0`, …). */
  category: '[data-testid="shots-category"]',
  weapon: '[data-testid="shots-weapon"]',
  quantity: '[data-testid="shots-shots"]',
...
test.describe('area: shot counts', () => {
  test('lists, filters, records, deletes and restores usages', async ({
    page,
  }) => {
    ...
    await expect(page.locator('.slim-table tbody tr').first()).toContainText(
      'Geissalp',
    );
    await page
    ...
    await expect
      .poll(() => page.locator(SHOTS.row).count())
      .toBeGreaterThan(10);
    ...
    await drawer
      .getByRole('button', { name: /Heute|Today|Aujourd|Oggi/ })
      .click();
    await drawer
      .getByRole('button', { name: /Vormittag|Morning|Matin|Mattina/ })
      .click();
    // First position: first category of the chosen room, then its first
    // combination (the form is n × combination + quantity, B1 6.2).
